...
def train(n=5, model=None, classifypath=None):
	print('testuz331')
	...
	X_test = load_temp_test_single(classifypath)
	...
	if model is None:
        ...
        print("final result is: ", predict_fn(X_test)[0])
        return predict_fn(X_test)[0]
	"""
	...
	if ('--help' in sys.argv) or ('-h' in sys.argv):
		...
		if len(sys.argv) > 2:
			kwargs['model'] = sys.argv[2]
                if len(sys.argv) > 3:
                        kwargs['classifypath'] = sys.argv[3]
		train(**kwargs)
